...
/**
 * license_key (string)
 * SET BY: /license/activate endpoint response (payload field) → stored in LicensePage.tsx
 * READ BY: All protected API calls (uploadResume, analyzeTarget, generatePrep, getPrepStatus, getLatestPrep)
 * PURPOSE: Authenticates student's license with backend, proves they've paid/been activated
 * BACKEND SECURITY: License must match student_id + company_name on backend
 * CLEARED ON: License deactivation, 403 Forbidden response, invalid license
 * BREAKS IF MISSING: All prepare/analyze/upload operations fail with 401/403
 * CRITICAL: This is the PRIMARY auth mechanism - never omit from API calls to backend
 */
export const SESSION_KEY_LICENSE_KEY = "license_key";
